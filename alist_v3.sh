...
if [ "$(id -u)" != "0" ]; then
  ...
  exit 1
elif ! command -v sudo servicectl >/dev/null 2>&1; then
...
INSTALL() {
  ...
  curl -L https://kgithub.com/Xhofe/alist/releases/latest/download/alist-linux-musl-$ARCH.tar.gz -o /tmp/alist.tar.gz $CURL_BAR
...
EOF
  ...
  # 添加开机启动
  sudo servicectl daemon-reload
  sudo servicectl enable alist >/dev/null 2>&1
}
...
#   echo -e "初始管理密码：${GREEN_COLOR}$(echo $get_password | awk -F'your password: ' '{print $2}')${RES}"
  ...
  echo -e "启动服务中"
  sudo servicectl restart alist

  echo
  echo -e "查看状态：${GREEN_COLOR}sudo servicectl status alist${RES}"
  echo -e "启动服务：${GREEN_COLOR}sudo servicectl start alist${RES}"
  echo -e "重启服务：${GREEN_COLOR}sudo servicectl restart alist${RES}"
  echo -e "停止服务：${GREEN_COLOR}sudo servicectl stop alist${RES}"
...
UNINSTALL() {
  ...
  sudo servicectl disable alist >/dev/null 2>&1
  sudo servicectl stop alist >/dev/null 2>&1
